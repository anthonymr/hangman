class Words
  @words = %w[
    ...
    fixable fjord flapjack flopping fluffiness flyby foxglove frazzled frizzled
    fuchsia funny gabby galaxy galvanize gazebo giaour gizmo glowworm glyph gnarly
    gnostic gossip grogginess haiku haphazard hyphen iatrogenic icebox injury
    ivory jackpot jaundice jawbreaker jaywalk jazziest jazzy jelly jigsaw jinx
    ...
    lengths lucky luxury lymph marquis matrix megahertz microwave mnemonic mystify
  ...
  class << self
    def get_new_word(dificulty = 0)
      return @words.sample if dificulty < 1

      dificulty = 7 if dificulty > 7

      @words.filter { |word| word.length == 3 + dificulty }.sample
    end
  end
end
